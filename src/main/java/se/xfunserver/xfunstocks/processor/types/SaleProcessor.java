...
import se.xfunserver.xfunstocks.events.StockSaleEvent;
...
public class SaleProcessor extends StockProcessor {
    ...
    public SaleProcessor(xFunStocks stockMarket,
    ...
    protected void processVault(ProcessorContext context) {
        stockMarket.getEconomy().depositPlayer(context.getPlayer(), context.getGrandTotal().doubleValue());
    }

    @Override
    protected void sendMessage(ProcessorContext context) {
        messages.sendSoldStockMessage(context.getPlayer(), context.getStock().getCompanyName(),
